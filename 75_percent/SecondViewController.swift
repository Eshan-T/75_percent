...
class SecondViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        {cell.firstSessionalMarks.text = marksSessionalOneMarks[indexPath.row]
            if marksSessionalTwoMarks.count == 0
            {
            ...
            if marksSessionalThreeMarks.count == 0
            {
    ...
    override func viewDidLoad() {
        ...
        {for index in 0...numberOfInternallyMarkedSubjects-1
        {
           ...
           {
               subjectInternalTotalArray.append(sessionalOneMarks[index])
              outOfAssesments = "15"
            }
            
            if numberOfAssessments == 2
            {
                subjectInternalTotalArray.append(sessionalOneMarks[index] + sessionalTwoMarks[index])
                outOfAssesments = "30"
            }
